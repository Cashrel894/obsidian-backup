...
4. 最后，进行**前向赋值**(Forward Assignment)，将序列中的变量依次取出，赋值为与其父节点一致的值即可。由于我们已经保证了弧一致性，因此每个子节点必然存在至少一个值与父节点一致，从而保证结果正确。

树状 CSP 算法可以通过**割集条件设置**(Cutset Conditioning) 拓展到图 CSP：
1. 首先，在图上找到一个最小子集，使得其补集构成一棵树，这个子集也称为**割集**。
2. 将这个子集作为图 CSP 问题进行搜索，对于搜索到的每个合法取值集合，都以其为约束，对剩余的树状 CSP 问题使用树状 CSP 算法求解，如果有解则解决，否则回溯。
最终，该算法的复杂度为 $\displaystyle O(d^c(n-c)d^{2})$，在 $\displaystyle c$ 较小时非常优秀。